...
def calc_RZ(rho, theta, theta_xpt, psi_data, psi_norm):
    """
    ...
    sep_pts = np.asarray(psi_data.get_lcfs_closed().coords)
    ...
    xpt_temp = psi_data.get_norm_xpt()

    for i, psi_norm_val in enumerate(psi_norm[:, 0]):
        if i == 0:
            R[i] = psi_data.get_mag_axis()[0]
            Z[i] = psi_data.get_mag_axis()[1]
        else:
            # attempt to draw flux surface line through that point
            # (may not work for flux surfaces very close to the magnetic axis)
            fs_line, fs_pts = draw_core_line(psi_data.R, psi_data.Z, psi_data.psi_norm_exp, psi_norm_val, sep_pts)
            ...
            for j, thetaval in enumerate(theta[0]):
                if psi_norm_val < 1.0:
                    ...
                    if thetaval == theta_xpt:
                        ...
                        thetaline = LineString([Point(psi_data.get_geom_axis()),
                                                Point([line_length * cos(thetaval) + psi_data.get_geom_axis()[0],
                                                       line_length * sin(thetaval) + psi_data.get_geom_axis()[1]])])
